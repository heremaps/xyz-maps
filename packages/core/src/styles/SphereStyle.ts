...
export interface SphereStyle {
    ...
    /**
     * The `light` property specifies a collection of light sources that can be used to illuminate `"Sphere"` features within the layer.
     ...
     * - `"buildingLights"` is a light group containing only an ambient light source. This group will only be used for features where {@link SphereStyle.light} is set to `"buildingLights"`.
     ...
     * - If {@link SphereStyle.light} is defined, it will reference a specific light group in `LayerStyle.lights`.
     * - If {@link SphereStyle.light} is not defined, the `"defaultLight"` light group (if specified) will be used.
