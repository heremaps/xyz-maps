...
function animateLine(timestamp) {
    ...
    if (elapsed > 1000 / fps) {
        if (coords.length == 0) {
            // revert all changes to navlink if one animation finishes
            editor.revert();
        } else {
            currentCoords.push(coords.pop());

            // append shape point to navlink
            navlink.coord(currentCoords);
        }
...
function editorReady(ready, nv, ov) {
    if (nv == true) {
        // get a navlink and its coordinates
        navlink = editor.getFeature('zyrABIMLWLaSHIkz', navlinkLayer);
        coords = navlink.coord();

        currentCoords = [coords.pop()];
