...
export type FactoryTaskResult = { buffers: GeometryBuffer[], pendingResources: Promise<any>[] };
...
type TaskData = {
    ...
    onDone?: (buffers: GeometryBuffer[]) => any
    // onDone?: (buffers: GeometryBuffer[]) => FactoryTaskResult
};
...
type TaskInput = {
    ...
    displayLayer: Layer,
    onInit?: () => void,
    onDone?: (result: FactoryTaskResult) => void
}


export class FactoryTask extends Task<TaskInput, TaskData> {
    name = 'CreateRenderBufferTask';
    time = EXCLUSIVE_TIME_MS;

    private factory: FeatureFactory;

    constructor(options: { priority?: number, factory: FeatureFactory }) {
        super(options);
        this.factory = options.factory;
    }

    init(data: TaskInput): TaskData {
        const {tile, displayLayer, onInit, onDone} = data;
        ...
        return {
            ...
            onDone(buffers) {
                const result = {buffers, pendingResources};
                onDone?.(result);
                return result;
            }
        };
    ...
    onDone(taskData?: TaskData) {
        ...
        const displayLayer = taskData.layer;
        const tile = taskData.tile;
                        ...
                        if (type == 'Line') {
                            ...
                            if (type == 'Polygon' || type == 'Extrude') {
                                ...
                                if (type == 'Text' || type == 'Icon') {
                                    ...
                                    const gradientTexture = this.factory.textureManager.getGradientTexture(gradient, HeatmapBuffer.verifyAndFixGradient);
                                ...
                                if (type === 'Terrain') {
                                    ...
                                    // terrain mode
                                    // if (shared.modelMode) (geoBuffer as ModelBuffer).hitTest = 1;
                                }
